...
const ProjectsPage = ({ data, location }) => {
  ...
  const quartz = data.quartz?.childImageSharp?.fixed
  const newspicks = data.newspicks?.childImageSharp?.fixed
  const gilt = data.gilt?.childImageSharp?.fixed
  ...
  return (
    <Layout location={location} title={siteTitle}>
      ...
      <p>
        {quartz && (<Image fixed={quartz} alt={`quartz-membership`}/>)}
      </p>
      ...
      <h2>NewsPicks</h2>
      <p>
        {quartz && (<Image fixed={newspicks} alt={`newspicks`}/>)}
      </p>
      ...
      <h2>Gilt Japan</h2>
      <p>
        {quartz && (<Image fixed={gilt} alt={`gilt-japan`}/>)}
      </p>
      <p>Coming soon...</p>
