...
投票機能のプロジェクトは決して技術的に難易度の高いものではなかったが、Shape Upを実践したことで普段とは少し違ったアプローチで問題に取り組むことができた。またこれは結果論にはなるが、同時期にパンデミックの影響で基幹となる事業が大打撃を受けており、事業整理を理由にこの投票機能はローンチからわずか数週間で廃止されることとなった。こうした結果は予想していなかったものの、開発する機能の重要度や期待度に応じて、開発リソースを適切に割り振ることができた（作り込みすぎるのを防げた）のはよかったと思う。品質やスピードの判断を個人がなんとなく勘や経験によって下すのではなく、フレームワークの一部としてシステマティックに実行できるのはShape Upやその他の開発メソッドを導入するメリットであると感じた。

いっぽう、6週間の開発サイクルなど制約が強すぎるために柔軟さに欠ける（仮に今すぐに実行したいアイデアがあっても最大で6週間、[Cool-down](https://basecamp.com/shapeup/2.2-chapter-08#cool-down)期間を合わせると8週間待たなければならない）点はデメリットとしてあげておきたい。また、開発者やデザイナーの自由度が高すぎる（自分たちでなんでも決められる）点は好みが分かれるかもしれない。加えてShape UpはBetting TableやSix-Week Cyclesなどプロダクトチームの外への影響も大きいため、特にC-レベルの意思決定層の合意を得ることが実行する上での鍵となるだろう。
