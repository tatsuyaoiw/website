...
プロジェクトのボトルネックはバックエンドの編成ツールの開発だったので、ここを出来るだけ素早く終えたい。Google Sheetsのスプレッドシートなら項目を自由に定義し、簡単なバリデーションも追加でき、かつ[Google Sheets API](https://developers.google.com/sheets/api)を使えばフロントエンドから直接コンテンツを読み込むことができる。編集者は多少不便ではあるが、最初はスプレッドシートに直接コンテンツを書き込んでもらう（ニュース編集者のひとりもプロジェクトのメンバーだ）。まさに斜め上の（悪い言い方をすればパッチワークのような）解決策だ。おそらくこのような提案は、Appetiteによる強い時間的予算が無ければ検討すらされなかっただろう。
